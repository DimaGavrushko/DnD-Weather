...
class LineChart extends React.Component {
  static uni = '';
  static selectedIndexx = 0;
  constructor (props) {
    super(props)
    LineChart.uni = this.props.units;
    //let main = − 32) × 5/9
    // let dataResault = this.props.data;
    // if (LineChart.uni === 'imperial') {
    //   dataResault = dataResault.map((item) => {
    //     console.log(item.x);
    //     let newY = Math.round((item.y - 32) * 5/9);
    //     let resultObj = {
    //       x: item.x,
    //       y: newY
    //     }
    ...
    this.changeStateGraphicScreen = this.changeStateGraphicScreen.bind(this);
    this.drawCoordinates = this.drawCoordinates.bind(this)
    this.drawCoordinate = this.drawCoordinate.bind(this)
    this.drawSelected = this.drawSelected.bind(this)
  }

  changeStateGraphicScreen(num) {
    console.log("Work OF FIRST");
    this.props.indexBarChart(num);
  }
        ...
        {seriesIndex === this.state.sortedData.length - 1 && (
          <TouchableWithoutFeedback onPress={() => {
            ...
            this.setState({
              ...
              LineChart.selectedIndexx = selectedIndex;
              console.log('TouchFirst and take index ' + selectedIndex);
              this.changeStateGraphicScreen(selectedIndex);
            })
    ...
    return (
      <TouchableWithoutFeedback key={key} onPress={() => {
        this.setState({selectedIndex: index}, () => {
          LineChart.selectedIndexx = selectedIndex;
          console.log('Touch and take index ' + selectedIndex);
          this.changeStateGraphicScreen(selectedIndex);
        })
     }}>
            ...
            {this.state.sortedData.map((series) => {
              ...
              return (
                <View key={series.seriesName}>
                  {dataObject.x ? (
                    <Text style={styles.tooltipTitle}>Time:{dataObject.x}</Text>
                ) : null}
                  <View style={{flexDirection: 'row', paddingLeft: 5, alignItems: 'center'}}>
                    ...
                    <Text style={styles.tooltipValue}>{numberWithCommas(dataObject.y, LineChart.uni, true)}</Text>
                  </View>
                </View>
              )
            })}
